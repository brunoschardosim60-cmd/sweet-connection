...
async function enviarWhatsapp(destino: string, mensagem: string) {
  ...
  if (!token || !phoneId || !to)
    return { status: "skipped", reason: "whatsapp_not_configured" } as const;
    ...
    handlers: {
      POST: async ({ request }) => {
        try {
          ...
          if (!tipos.has(type) || !/^[0-9a-f-]{36}$/i.test(id))
          ...
          await supabaseAdmin
            ...
            .update({
              ...
              last_error: "reason" in result ? result.reason : null,
            ...
            try {
              ...
              await supabaseAdmin
                ...
                .update({
                  ...
                  last_error: "reason" in whatsResult ? whatsResult.reason : null,
          ...
          return Response.json(
            { error: "Não foi possível encaminhar a notificação." },
            { status: 503 },
          );
        }
      },
