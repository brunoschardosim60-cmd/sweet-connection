-- Corrige a função já aplicada: o enum da Nexa usa "publicado", não "published".
...
begin
  select id, published_content into v_site from public.minisites where slug=lower(trim(requested_slug)) and status='publicado' and published_content is not null;
