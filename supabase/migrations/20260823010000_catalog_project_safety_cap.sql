-- Limite operacional interno para evitar abuso no plano Catálogo.
-- Não é divulgado na interface comercial: é uma proteção de capacidade.
...
begin
  ...
  allowed_count := case
    when coalesce(active, false) and tier = 'catalog' then 50
    when coalesce(active, false) and tier = 'professional' then 3
    else 1
  end;
