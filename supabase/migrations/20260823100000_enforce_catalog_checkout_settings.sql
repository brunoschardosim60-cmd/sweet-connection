...
begin
  ...
  if requested_modalidade='entrega' then
    ...
    select coalesce((value->>'taxa')::numeric,v_taxa) into v_taxa from jsonb_array_elements(coalesce(v_site.published_content->'comercio'->'taxasPorBairro','[]'::jsonb)) where lower(trim(value->>'bairro'))=lower(v_bairro) limit 1;
  end if;
